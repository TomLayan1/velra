...
const Navbar:React.FC = () => {
  ...
  return (
    <>
      {showMenu && <div className='w-full h-[100vh] bg-[#d6d4d4c5] lg:hidden fixed top-0 left-0 z-30'></div>}
      <div className='text-white lg:border-t-5 border-white'>
        <div className='container p-4 md:px-8 lg:py-0 mx-auto flex justify-between items-center'>
          <h1 className='text-4xl'>Velra</h1>
          <div className={`bg-white z-40 fixed lg:static top-0 right-0 w-[50%] lg:w-auto h-[100vh] lg:h-16 pt-15 lg:pt-0 duration-300 lg:translate-x-0 ease-in-out ${showMenu ? 'translate-x-0' : 'translate-x-full'} text-black flex lg:items-center px-7 rounded-bl-3xl rounded-br-3xl`}>
            <nav className='flex flex-col lg:flex-row gap-5 lg:gap-7'>
              {navLinks?.map(link => (
                <li className='list-none' key={link.id}>{link.nav}</li>          
              ))}
            </nav>
          </div>
